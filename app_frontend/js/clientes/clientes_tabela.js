function renderTabela(clientes){
    ...
    for(var i in clientes){
        ...
        if (parseInt(clientes[i].locacao, 10) >= "3") {
        ...
        str+=`<tr>
                ...
                <td>${clientes[i].matricula}</td>
                <td>${clientes[i].nome}</td>
                <td>(${clientes[i].telefone.substr(0, 2)})-${clientes[i].telefone.substr(2, 4)}-${clientes[i].telefone.substr(6)}</td>
                <td>${disponibilidade} ${clientes[i].locacao}</td>
                <td><button type="button" class="edit btn btn-secondary" data-id_cliente="${clientes[i].id_cliente}">Editar</button></td>
                <td><button type="button" class="delete btn btn-danger" data-id_cliente="${clientes[i].id_cliente}">Deletar</button></td>
            </tr>`;
    } 
    str+= `
    </table>
    ...
    <path d="M8 0a1 1 0 0 1 1 1v6h6a1 1 0 1 1 0 2H9v6a1 1 0 1 1-2 0V9H1a1 1 0 0 1 0-2h6V1a1 1 0 0 1 1-1z"/></svg> Inserir Cliente</button></div></div>
    </div>
    </div>`
    ;
    ...
    for(let linkEdit of linksEdit) {
        linkEdit.onclick = function(event){
            onEdit(event.target.getAttribute("data-id_cliente"));
        }
    }
    ...
    for(let linkDelete of linksDelete) {
        linkDelete.onclick = function(event){
            onDeletar(event.target.getAttribute("data-id_cliente"));
        }
    }
}
