/* function renderTabela(locacoes, clientes, livros){
    var str=`
    <div class="container-md rounded-3 border border-3">
    <h3>Locações</h3>
    ...
    for(var i = locacoes.length - 1; i >= 0; i--){
        for(var x in clientes){
            if (locacoes[i].id_cliente === clientes[x].id_cliente) {
                var nomedocliente = clientes[x].nome
        ...
        if(!locacoes[i].data_real_entrega) {
            var dataentrega = "xxxx-xx-xx"
            }
        else{
            dataentrega = locacoes[i].data_real_entrega.substr(0, 10)
        };
        ...
        str+=`<tr>
                ...
                <td> <b class="text-${corlocacaodocliente}">${locacaodocliente}</b> <b>|</b> ${nomedocliente}</td>
                <td> <b class="text-${corquantidadedolivro}">${quantidadedolivro}</b> <b>|</b> ${nomedolivro}</td>
                <td>${locacoes[i].data_retirada.substr(0, 10)}</td>
                <td>${locacoes[i].data_estimada_entrega.substr(0, 10)}</td>
                <td>${dataentrega}</td>
    ...
    <div class="col-4"><div class="col-sm-4"><button id='novo' type="button" class="col-auto btn btn-dark">Nova Locação</button></div></div>
    </div>
    </div>`
    ;
